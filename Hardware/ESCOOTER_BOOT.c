...
void Boot_Init()
{
    ...
    if(resetReason == PowerCC26X2_RESET_SHUTDOWN_IO)
    {
        ...
        BootCountDownCreate();
        BootCountDownInit();
        GPIO_setConfig(CONFIG_GPIO_BTN2_CONST, GPIO_CFG_IN_PU);
        uint8_t press = GPIO_read(CONFIG_GPIO_BTN2_CONST);
        BootServiceRoutine(press);
        BootCountEliminate();
        GPIO_resetConfig(CONFIG_GPIO_BTN2_CONST);
        howToBoot = 0x00;
    }
...
void BootServiceRoutine(uint8_t isWakeUp)
{
   ...
   if(isWakeUp == 0)
   {
         ...
         uint8_t stillPress = GPIO_read(CONFIG_GPIO_BTN2_CONST);
         if(stillPress == 0x00)
         {
         ...
         while(BOOT_FINISHED == 0x00)
         {
              ...
              */
             uint8_t status = GPIO_read(CONFIG_GPIO_BTN2_CONST);
...
void SystemShutDownRoutine()
{
    GPIO_resetConfig(CONFIG_GPIO_BTN2_CONST);
    /* How to wake me up??  */
    GPIO_setConfig(CONFIG_GPIO_BTN2_CONST, GPIO_CFG_INPUT_INTERNAL | GPIO_CFG_PULL_UP_INTERNAL | GPIO_CFG_SHUTDOWN_WAKE_LOW);
    /* Let me sleep! Bye!  */
    Power_shutdown(0, 0);
    while(1)
    {
